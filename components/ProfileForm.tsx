...
import { ArrowRight, TrendingDown, Scale, TrendingUp, ChevronDown, AlertCircle } from "lucide-react";
      ...
      <div className="max-w-[480px] w-full pt-8 pb-12 px-6 sm:px-10">
        ...
        <div className="text-center mb-10">
          <div className="flex items-center justify-center gap-1.5 mb-6">
            <span className="material-symbols-outlined filled-icon text-[#084C3A] text-4xl">local_fire_department</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Giới tính */}
          <div>
            <label className="block text-sm font-bold text-[#084C3A] uppercase tracking-wide mb-2">Giới tính</label>
              ...
              {['male', 'female'].map((g) => (
                <button
                  ...
                  className={`flex-1 py-3.5 text-sm font-semibold rounded-lg transition-all ${
                    form.gender === g ? "bg-white text-[#084C3A] shadow-sm" : "text-gray-500 hover:text-[#084C3A]"
                  }`}
                >
                  {g === 'male' ? 'Nam' : 'Nữ'}
                </button>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={item.key}>
                <label className="block text-sm font-bold text-[#084C3A] uppercase tracking-wide mb-2">{item.label}</label>
                <input
                  type="number"
                  min="1"
                  max={item.max}
                  ...
                  className={`w-full bg-white text-[#1A2F24] text-base font-semibold px-4 py-3.5 rounded-xl focus:outline-none border shadow-sm transition-all ${
                    +form[item.key as keyof typeof form] <= 0 ? "border-red-500" : "border-transparent focus:border-[#084C3A]"
                  }`}
                />
              </div>
            ))}
          ...
          <div>
            <label className="block text-sm font-bold text-[#084C3A] uppercase tracking-wide mb-2">MỨC ĐỘ HOẠT ĐỘNG</label>
          ...
          <div>
            <label className="block text-sm font-bold text-[#084C3A] uppercase tracking-wide mb-2">MỤC TIÊU CỦA BẠN</label>
              ...
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${
                    form.goal === g.v ? "bg-[#E6F4EA] border-[#084C3A] text-[#084C3A]" : "bg-white border-transparent text-[#1A2F24] shadow-sm"
                  }`}
                >
          ...
          <div className="relative overflow-hidden bg-[#F2F8F4] p-6 rounded-2xl border border-[#E3EFE8]">
            <span className="material-symbols-outlined absolute -bottom-6 -right-4 text-[130px] text-[#084C3A] opacity-[0.04] pointer-events-none transform -rotate-12">energy_savings_leaf</span>
            <div className="relative z-10 mb-4">
              <p className="text-[11px] font-bold text-[#4A6355] uppercase tracking-wide mb-1">NHU CẦU DINH DƯỠNG DỰ TÍNH</p>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black text-[#084C3A] font-numbers">{error ? "--" : results.calories.toLocaleString()}</span>
              ...
              {[
                ...
                <div key={m.label}>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center`} style={{ backgroundColor: m.bg }}>
                      <span className="material-symbols-outlined filled-icon text-[12px]" style={{ color: m.color }}>{m.icon}</span>
                    </div>
                    <p className="text-[12px] font-bold text-[#4A6355]">{m.label}</p>
                  </div>
                  <p className="text-[22px] font-black text-[#1A2F24] font-numbers leading-none">{error ? "0" : m.val}g</p>
                </div>
              ))}
            </div>
          </div>
