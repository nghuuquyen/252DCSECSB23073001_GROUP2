...
const AddMealModal = dynamic(
  () => import('@/components/diary/AddMealModal'),
  {
...
function formatDate(dateStr: string) {
  ...
  return { day: date.getDate(), month: date.getMonth() + 1, isToday: dateStr === today };
}
...
const MealSection = memo(function MealSection({ mealType, meal, onAdd }: {
  mealType: MealType;
...
export default function DiaryPage() {
  ...
  const goNext     = useCallback(() => { if (currentDate !== getToday()) setCurrentDate((d) => offsetDate(d, 1)); }, [currentDate]);
  ...
  const handleAddIngredient = useCallback((mealType: MealType, ingredient: Ingredient) => {
    ...
    <div className="bg-background text-on-background font-body-md min-h-screen">
      ...
      <main className="pt-16 pb-28 sm:pb-24 px-4 sm:px-6 max-w-[1100px] mx-auto">
        ...
        <nav className="flex items-center justify-center gap-4 sm:gap-8 my-4 sm:my-5">
          <button onClick={goBack} className="p-2 hover:bg-surface-container rounded-full transition-colors group shrink-0">
          ...
          <button onClick={goNext} disabled={isAfterToday} className="p-2 hover:bg-surface-container rounded-full transition-colors group disabled:opacity-30 shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5 items-start">
          ...
          <div className="lg:col-span-5 space-y-3">
            ...
            {MEAL_ORDER.map((type) => (
              <MealSection key={type} mealType={type} meal={getMeal(type)} onAdd={openModal} />
            ))}
          </div>
